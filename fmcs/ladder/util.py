...
from .models import Matches, MatchupStatsNode, PlayerStatsNode, RatingPeriod, Players, PlayerRatingNode
...
def glicko_reprocess_all_stats(reset_id_counter=True):
    ...
    for game in Matches.objects.order_by("date_played"):
        game.process_game()
...
def glicko_process_new_ratings():
    ...
    if latest_rating_period:
        ...
        earliest_game = Matches.objects.last()
...
def ladder_reprocess_all_ratings(reset_id_counter=True):
    ...
    if reset_id_counter:
        with connection.cursor() as cursor:
            cursor.execute(
                "ALTER SEQUENCE ladder_playerratingnode_id_seq RESTART with 1"
            )
            cursor.execute(
                "ALTER SEQUENCE ladder_ratingperiod_id_seq RESTART with 1"
            )
...
def ladder_clear_player_table(self):
    try:
    ...
    try:
        with open(csv_file_path, newline='') as csvfile:
            ...
            for row in reader:
                ...
                player_a, created_a = Players.objects.get_or_create(name=player_a_name)
                player_b, created_b = Players.objects.get_or_create(name=player_b_name)
